...
class bookaroom_events {
	...
	protected static function editEvent( $dateList, $delete = array() )
	{
		...
		$sql = "UPDATE `$table_name` SET 
			...
			`ev_website` = '".$_SESSION['bookaroom_meetings_externalVals']['website']."', 
			`ev_webText` = '".$_SESSION['bookaroom_meetings_externalVals']['websiteText']."', 
			`ev_regContactType` = '".$_SESSION['bookaroom_meetings_externalVals']['regContactType']."'
			WHERE `res_id` = '{$eventInfo['res_id']}'";
	...
	protected static function addEvent( $dateList, $delete = array() )
	{
		...
		$sql = "INSERT INTO `$table_name` ( `res_created`, `ev_desc`, `ev_maxReg`, `ev_amenity`, `ev_waitingList`, `ev_presenter`, `ev_privateNotes`, `ev_publicEmail`, `ev_publicName`, `ev_publicPhone`, `ev_noPublish`, `ev_regStartDate`, `ev_regType`, `ev_submitter`, `ev_title`, `ev_website`, `ev_webText`, `ev_regContactType` ) 
			VALUES(
            '".$currentTimeMySQL."', 
			...
			'".$_SESSION['bookaroom_meetings_externalVals']['regContactType']."' )";
		$wpdb->query( $sql );
	...
	function checkForm( &$externals, $roomContList, $branchList, $amenityList )
	{
		...
		if( $externals['registration'] !== 'no' ) {
			...
			# reg contact type
			if( empty( $externals['regContactType'] ) ) {
				$final[] = __( 'You must choose a registration contact type.', 'book-a-room' ); 
				$errorBG['regContactType'] = true;	
			} else {
				...
				if( !array_key_exists( $externals['regContactType'], $good_arr ) ) {
					$final[] = __( 'You must choose a registration contact type.', 'book-a-room' ); 
					$errorBG['regContactType'] = true;
				}
			}
		...
		# categories
		if( empty( $externals['category'] ) or count( $externals['category'] ) < 1 ) {
			$final[] = __( 'You must choose at least one category.', 'book-a-room' ); 
			$errorBG['category'] = true;
		}
		
		# age groups
		if( empty( $externals['ageGroup'] ) or count( $externals['ageGroup'] ) < 1 ) {
			$final[] = __( 'You must choose at least one age group.', 'book-a-room' );
			$errorBG['ageGroup'] = true;
		}
	...
	function checkID( $eventID )
	{
		...
		$sql = "SELECT `ti`.`ti_id`, `ti`.`ti_type`, `ti`.`ti_extID`, `ti`.`ti_created`, `ti`.`ti_extraInfo`, 
			...
			`ti`.`ti_attNotes`, `ev`.`ev_regContactType`,
	...
	function fixSavedEventInfo( $eventInfo, $externals )
	{
		$goodArr = array( 'roomID' => 'ti_roomID', 
						...
						'eventTitle' => 'ev_title',  
						'maxReg' => 'ev_maxReg',  
						'regContactType' => 'ev_regContactType',  
	...
	# Pull in POST and GET values
	{
		...
		$postArr = array(	
							...
							'regDate'					=> FILTER_SANITIZE_STRING, 'regDate'					=> FILTER_SANITIZE_STRING, 
							'regID'						=> FILTER_SANITIZE_STRING, 
							...
							'waitingList'				=> FILTER_VALIDATE_INT, 
							...
							'regContactType'			=> FILTER_SANITIZE_STRING, 
																 );
		...
		foreach( $arrayCheck as $key ) {
			if( isset( $final[$key] ) and $final[$key] == "0" ) {
				$final[$key] = "0";
			} elseif( empty( $final[$key] ) ) {
				$final[$key] = NULL;
