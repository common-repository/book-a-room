...
add_action( 'init', 'bookaroom_script_enqueuer' );
...
function bookaroom_script_enqueuer() {
	...
	wp_enqueue_script( 'bookaroom_js', plugins_url( 'book-a-room/js/jstree/jquery.jstree.js' ), array('jquery'), null, true );
...
function update_bookaroom_database()
{
	...
	if( get_option( 'bookaroom_db_version' ) <= '3' ) {
		
		$sql = "SHOW COLUMNS from {$wpdb->prefix}bookaroom_reservations LIKE 'ev_regContactType'";
		if( false == $wpdb->query( $sql ) ) {
		
			$sql = "ALTER TABLE {$wpdb->prefix}bookaroom_reservations ADD `ev_regContactType` enum('both','phone','either','none','email') CHARACTER SET latin1 DEFAULT 'either';";
			$wpdb->query( $sql );
			
			$sql = "ALTER TABLE {$wpdb->prefix}bookaroom_reservations_deleted ADD `ev_regContactType` enum('both','phone','either','none','email') CHARACTER SET latin1 DEFAULT 'either';";
			$wpdb->query( $sql );
		
		}

	}
	update_option( 'bookaroom_db_version', 4 );
}
...
# simple class for activating, deactivating and uninstalling plugin
{
	...
	# this is only run when hooked by activating plugin
    {
		...
		$sql = "CREATE TABLE {$wpdb->prefix}bookaroom_reservations (
				  ...
				  ev_regContactType enum('both','phone','either','none','email') DEFAULT 'either', 
	...
	# this is only run when hooked by uninstalling plugin
    {
		...
		global $bookaroom_db_version;
		 ...
		 $roles = get_editable_roles();
        foreach ($GLOBALS['wp_roles']->role_objects as $key => $role) {
            if (isset($roles[$key]) && $role->has_cap('bookaroom_admin')) {
                $role->remove_cap('bookaroom_admin');
            }
        }
...
# main settings functions
{
	public static function add_settingsPage()
	{
		...
		# Daily Schedule
		add_menu_page( __( 'Book a Room Daily Schedules', 'book-a-room' ), __( 'Daily Schedules', 'book-a-room' ), 'read', 'bookaroom_daily_schedules', array( 'book_a_room_meetings', 'bookaroom_dailyMeetings' )  );
        ...
        add_submenu_page( 'bookaroom_daily_schedules', __( 'Meeting Room Meetings - Contact List', 'book-a-room' ), __( 'Contact List', 'book-a-room' ), 'bookaroom_admin', 'bookaroom_daily_schedules_contact',  array( 'book_a_room_meetings', 'bookaroom_contactList' ) );
        
        remove_submenu_page('bookaroom_daily_schedules','bookaroom_daily_schedules');
...
#		add_submenu_page( 'bookaroom_meetings', "", '<span style="display:block; margin:1px 0 1px -5px; padding:0; height:2px; line-height:1px; background:#DDD;"></span>', "read", "#" );
		...
		# Manage events
		add_menu_page( __( 'Book a Room Event Settings', 'book-a-room' ), __( 'Event Settings', 'book-a-room' ), 'bookaroom_admin', 'bookaroom_event_settings', array( 'bookaroom_settings_age', 'showFormAge' ) );
		
		add_submenu_page( 'bookaroom_event_settings', __( 'Manage Age Groups', 'book-a-room' ), __( 'Manage Age Groups', 'book-a-room' ), 'bookaroom_admin', 'bookaroom_event_settings',  array( 'bookaroom_settings_age', 'showFormAge' ) );

		add_submenu_page( 'bookaroom_event_settings', __( 'Manage Categories', 'book-a-room' ), __( 'Manage Categories', 'book-a-room' ), 'bookaroom_admin', 'bookaroom_event_settings_categories',  array( 'bookaroom_settings_categories', 'showFormCategories' ) );
				
		# Manage Meeting Room settings
		add_menu_page( __( 'Book a Room Settings', 'book-a-room' ), __( 'Meeting Room Settings', 'book-a-room' ), 'bookaroom_admin', 'bookaroom_Settings', array( 'bookaroom_settings_admin', 'bookaroom_admin_admin' ) );

		add_submenu_page( 'bookaroom_Settings', __( 'Meeting Room Settings', 'book-a-room' ), __( 'Settings', 'book-a-room' ), 'bookaroom_admin', 'bookaroom_Settings',  array( 'bookaroom_settings_admin', 'bookaroom_admin_admin' ) );
		
		add_submenu_page( 'bookaroom_Settings', __( 'Meeting Room Settings - Amenities', 'book-a-room' ), __( 'Amenities Admin', 'book-a-room' ), 'bookaroom_admin', 'bookaroom_Settings_Amenities',  array( 'bookaroom_settings_amenities', 'bookaroom_admin_amenities' ) );
		
		add_submenu_page( 'bookaroom_Settings', __( 'Meeting Room Settings - Branches', 'book-a-room' ), __( 'Branch Admin', 'book-a-room' ), 'bookaroom_admin', 'bookaroom_Settings_Branches',  array( 'bookaroom_settings_branches', 'bookaroom_admin_branches' ) );

		add_submenu_page( 'bookaroom_Settings', __( 'Meeting Room Settings - Rooms', 'book-a-room' ), __( 'Room Admin', 'book-a-room' ), 'bookaroom_admin', 'bookaroom_Settings_Rooms',  array( 'bookaroom_settings_rooms', 'bookaroom_admin_rooms' ) );

		add_submenu_page( 'bookaroom_Settings', __( 'Meeting Room Settings - Room Containers', 'book-a-room' ), __( 'Containers Admin', 'book-a-room' ), 'bookaroom_admin', 'bookaroom_Settings_RoomCont',  array( 'bookaroom_settings_roomConts', 'bookaroom_admin_roomCont' ) );
				
		add_submenu_page( 'bookaroom_Settings', __( 'Meeting Room Settings - City Management', 'book-a-room' ), __( 'City Admin', 'book-a-room' ), 'bookaroom_admin', 'bookaroom_Settings_cityManagement',  array( 'bookaroom_settings_cityManagement', 'bookaroom_admin_mainCityManagement' ) );
				
		add_submenu_page( 'bookaroom_Settings', "", '<span style="display:block; margin:1px 0 1px -5px; padding:0; height:2px; line-height:1px; background:#DDD;"></span>', "bookaroom_admin", "#" );
		
		add_submenu_page( 'bookaroom_Settings', __( 'Meeting Room Settings - Email', 'book-a-room' ), __( 'Email Admin', 'book-a-room' ), 'bookaroom_admin', 'bookaroom_Settings_Email',  array( 'bookaroom_settings_email', 'bookaroom_admin_email' ) );
		
		add_submenu_page( 'bookaroom_Settings', __( 'Meeting Room Settings - Content', 'book-a-room' ), __( 'Content Admin', 'book-a-room' ), 'bookaroom_admin', 'bookaroom_Settings_Content',  array( 'bookaroom_settings_content', 'bookaroom_admin_content' ) );

		add_submenu_page( 'bookaroom_Settings', __( 'Meeting Room Settings - Closings', 'book-a-room' ), __( 'Closings Admin', 'book-a-room' ), 'bookaroom_admin', 'bookaroom_Settings_Closings',  array( 'bookaroom_settings_closings', 'bookaroom_admin_closings' ) );
		
		add_submenu_page( 'bookaroom_Settings', __( 'Meeting Room Settings - Locale', 'book-a-room' ), __( 'Locale Admin', 'book-a-room' ), 'bookaroom_admin', 'bookaroom_Settings_Locale',  array( 'bookaroom_settings_locale', 'bookaroom_admin_locale' ) );


		# Help files
		add_menu_page( __( 'Bookaroom Help', 'book-a-room' ), __( 'Bookaroom Help', 'book-a-room' ), 'bookaroom_admin', 'Bookaroom_Help', array( 'bookaroom_help', 'showHelp' ) );

		add_submenu_page( 'Bookaroom_Help', __( 'Meeting Room Help', 'book-a-room' ), __( 'Main Help', 'book-a-room' ), 'bookaroom_admin', 'Bookaroom_Help',  array( 'bookaroom_help', 'showHelp' ) );
