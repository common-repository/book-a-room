...
<script language="JavaScript" type="text/javascript">
	...
	$( function () {
		...
		$( 'input[name=registration]:radio' ).change( function () {
			if ( $( this ).val() == 'no' ) {
				$( '#waitingList, #maxReg, #regDate, #regContactType' ).prop( 'disabled', true );
			} else {
				$( '#waitingList, #maxReg, #regDate, #regContactType' ).prop( 'disabled', false );
			}
		} );
...
?>		
	...
	<table class="tableMain">
		...
		<tr>
			<td>
				<p><?php _e( 'Event Description', 'book-a-room' ); ?><br>
					<em> <?php _e( '(1000 char. max)', 'book-a-room' ); ?></em>
				</p>
			</td>
			<td<?php if( !empty( $errorArr['errorBG']['eventDesc'] ) ) echo ' class="error"'; ?>><textarea name="eventDesc" cols="55" rows="6" id="eventDesc" onKeyDown="limitText(this.form.eventDesc,this.form.eventDesc_countdown,1000);" onKeyUp="limitText(this.form.eventDesc,this.form.eventDesc_countdown,1000);"><?php echo $externals['eventDesc']; ?></textarea>
				<br>
				<input readonly type="text" name="eventDesc_countdown" size="3" value="1000" id="numbCount"> <?php /* translators: this appears as "n Characters left" under a text box. */ _e( 'Characters left.', 'book-a-room' ); ?></td>
		</tr>
		<?php
		...
		<tr>
			<td><?php _e( 'Registration Contact Type', 'book-a-room' ); ?></td>
			<td<?php if( !empty( $errorArr['errorBG']['regContactType'] ) ) echo ' class="error"'; ?>>
				<select name="regContactType" id="regContactType">
				<?php
				$good_arr = [ 'either' => __( 'Either required', 'book-a-room' ), 'none' => __( 'None Required', 'book-a-room' ), 'email' => __( 'Email Required', 'book-a-room' ), 'phone' => __( 'Phone Required', 'book-a-room' ), 'both' => __( 'Both Required', 'book-a-room' ) ];
				
				$selected = ( !array_key_exists( $externals['regContactType'], $good_arr ) ) ? ' selected="selected"' : null;
				echo "<option value=\"\"{$selected}>".__( 'Choose one', 'book-a-room' )."</option>";
				
				foreach( $good_arr as $key => $val ) {
					$selected = ( $externals['regContactType'] == $key ) ? ' selected="selected"' : null;
		...
		<tr>
			...
			<td<?php if( !empty( $errorArr['errorBG']['maxReg'] ) ) echo ' class="error"'; ?>><input name="maxReg" type="number" id="maxReg" value="<?php echo $externals['maxReg']; ?>" size="3" maxlength="3" />
			</td>
		</tr>
		<tr>
			<td><?php _e( 'Waiting List', 'book-a-room' ); ?></td>
			<td<?php if( !empty( $errorArr['errorBG']['maxReg'] ) ) echo ' class="error"'; ?>><input name="waitingList" type="number" id="waitingList" value="<?php echo $externals['waitingList']; ?>" size="3" maxlength="3" />
			</td>
		</tr>
		<tr>
