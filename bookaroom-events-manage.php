...
class bookaroom_events_manage
{
	...
	function getEventList( $externals, $amenityList, $branchList, $roomContList, $roomList )
	{
		...
		`ti`.`ti_id`, `ti`.`ti_startTime`, `ti`.`ti_endTime`, `ti`.`ti_type`, `res`.`ev_title`, `res`.`ev_desc`, `ti`.`ti_roomID`, `ti`.`ti_noLocation_branch`, `ti`.`ti_extraInfo`, `res`.`ev_regType`, `res`.`ev_maxReg`, `res`.`ev_noPublish`, COUNT( DISTINCT `tiCount`.`ti_id` ) as eventCount, `res`.`res_id`, `res`.`ev_regContactType` 
			FROM `{$wpdb->prefix}bookaroom_times` AS `ti`
